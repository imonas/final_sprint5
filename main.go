...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	meanSpeedMps := w.meanSpeed() * KmHInMsec // Скорость в м/с
	heightMeters := float64(w.Height) / CmInM // Рост в метрах
	durationHours := w.Duration.Hours()       // Длительность в часах

	if heightMeters == 0 { // Избегаем деления на ноль
		return 0.0
	}

	calories := (CaloriesWeightMultiplier*w.Weight + (math.Pow(meanSpeedMps, 2)/heightMeters)*CaloriesSpeedHeightMultiplier*w.Weight) * durationHours * MinInHours
	return calories
}
...
func (s Swimming) meanSpeed() float64 {
	// вставьте ваш код ниже
	totalDistanceMeters := float64(s.LengthPool * s.CountPool)
